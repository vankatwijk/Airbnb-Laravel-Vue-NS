...
function onRequestComplete(requestId, result) {
    ...
    callbacks.resolveCallback({
        content: {
            raw: result.raw,
            toArrayBuffer: function () { return Uint8Array.from(result.raw.toByteArray()).buffer; },
...
function buildJavaOptions(options) {
    ...
    if (typeof options.content === "string" || options.content instanceof FormData) {
        var nativeString = new java.lang.String(options.content.toString());
        var nativeBytes = nativeString.getBytes("UTF-8");
        var nativeBuffer = java.nio.ByteBuffer.wrap(nativeBytes);
        javaOptions.content = nativeBuffer;
    }
    else if (options.content instanceof ArrayBuffer) {
        var typedArray = new Uint8Array(options.content);
        var nativeBuffer = java.nio.ByteBuffer.wrap(Array.from(typedArray));
        javaOptions.content = nativeBuffer;
    }
